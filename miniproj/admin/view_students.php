...
// Delete individual student securely
if(isset($_GET['id'])) {
    $id = intval($_GET['id']);
...
// Fetch all students
$result = $conn->query("SELECT id, name, email, status FROM students ORDER BY id DESC");
...
<form method="post" action="delete_inactive_students.php" onsubmit="return confirm('Delete all inactive students?');">
    <button type="submit">Delete Inactive Students</button>
</form>
...
<tr class="<?php echo $row['status']=='inactive'?'inactive':''; ?>">
    ...
    <td>
        ...
        <a href="view_students.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Delete this student?')">Delete</a>
    </td>
</tr>
<?php endwhile; ?>
</tbody>
